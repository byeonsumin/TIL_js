...
// for 문의 초기변수로 자주 사용하는 변수명 : i, j, k, m, n
...
//querySelectopAll 함수로 선택한 DOM 변수에도 for문을 많이 활용한다.
//배열 인덱스 for문
...
console.log(array[1])
console.log(array[2]) */
for(let i=0; i<array.length; i++){ //0으로 시작해서 배열의 길이(length)까지 출력해라
    console.log(array[i])
}
console.log('-------------------------------------------------')
//css ~ photoshop까지 역순으로 콘솔 출력
for(let i=array.length-1; i>=0; i--){ //인덱스는 0부터 시작하니까 -1을 해주어야함
    console.log(array[i])
}
console.log('-------------------------------------------------')
const flower = [{
...
// css or sass 클래스를 미리 준비하고 js 동적 결과에 따라 적용
for(let i=0; i<dan2Li.length; i++){
...
console.log('-------------------------------------------------')
//while문(반복횟수가 정해지지 않고 특정 조건이 참일동안 반복실행)
//활용 예) 사용자로부터 올바른 입력값을 받을때까지 반복
//활용 예) 웹서비스 및 게임 등에서 계속 실행되어야 하는 프로그램
//while(조건){조건이 참일때 반복실행}

/* let htmlQ = prompt('a태그는 가로, 세로 크기를 가질 수 없다. ox중 하나를 입력하세요')//정답은 x
//정답을 맞출때까지 무한반복질문
while(htmlQ != 'x'){ //정답이 x가 아니면 반복할거다
    htmlQ = prompt('a태그는 가로, 세로 크기를 가질 수 없다. ox중 하나를 입력하세요')//정답은 x
}
alert('정답입니다') */

/* let cssQ = prompt('ox 퀴즈 : align-item은 grid에 쓸 수 없다.')//x
while(cssQ != 'x'){
    cssQ = prompt('ox 퀴즈 : align-item은 grid에 쓸 수 없다.')
}
alert('정답입니다') */

while(false){
    const qa = prompt('자바스크립트는 동적언어이다(O/X)') //O
    if(qa !== 'o' && qa !== 'x'){ //잘못된 답안 조건문
        alert('잘못된 입력입니다. O, X만 입력해주세요')
        continue; //다음 구문을 생략하고 반복문의 처음부터 다시 시작한다.
        //break;//while 문이 강제 종료
    }
    //정답, 오답 세트
    if(qa === 'o'){
        alert('정답입니다'); break;
...
for(let i in cat){ //i에 cat정보를 넣는다
    console.log(i, cat[i])
}
